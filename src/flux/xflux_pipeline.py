...
class XFluxPipeline:
    ...
    def set_controlnet(self, control_type: str, local_path: str = None, repo_id: str = None, name: str = None):
        ...
        # For image restoration use-cases, we don't run any annotator
        if control_type in ("raw", "sr", "lq", "identity"):
            self.annotator = None
        else:
    ...
    def __call__(self,
        ...
        # In image restoration mode, force the working resolution to 1024x1024
        if self.controlnet_loaded and getattr(self, "control_type", None) in ("raw", "sr", "lq", "identity"):
        ...
        if self.controlnet_loaded:
            # Direct image conditioning for restoration tasks
            if getattr(self, "control_type", None) in ("raw", "sr", "lq", "identity"):
                if isinstance(controlnet_image, Image.Image):
                    controlnet_image = controlnet_image.convert("RGB").resize((width, height))
                    controlnet_image = np.array(controlnet_image)
                else:
                    # Fallback: expect numpy array in HWC, convert to RGB-like 3 channels if needed
                    controlnet_image = np.array(controlnet_image)
                    if controlnet_image.ndim == 2:
                        controlnet_image = np.stack([controlnet_image] * 3, axis=-1)
                    if controlnet_image.shape[-1] == 4:
                        controlnet_image = controlnet_image[..., :3]
                    controlnet_image = np.ascontiguousarray(
                        np.array(Image.fromarray(controlnet_image).resize((width, height)))
                    )
