...
def get_obj_from_str(string: str, reload: bool = False) -> Any:
    ...
    if module.startswith("diffbir."):
        full_module_path = "custom_nodes.ComfyUI_LucidFlux.src.DiffBIR." + module #
    else:
        full_module_path = module
    if reload:
        try:
            module_imp = importlib.import_module(full_module_path)
            importlib.reload(module_imp)
        except ModuleNotFoundError:
            module_imp = importlib.import_module(module)
            importlib.reload(module_imp)
    try:
        return getattr(importlib.import_module(full_module_path, package=None), cls)
    except ModuleNotFoundError:
...
def load_model_from_file(path: str) -> Dict[str, torch.Tensor]:
