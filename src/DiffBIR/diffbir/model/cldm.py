...
class ControlLDM(nn.Module):
    ...
    def vae_encode(
        ...
        if image.device != next(self.vae.parameters()).device:
                image = image.to(next(self.vae.parameters()).device)
    ...
    def forward(self, x_noisy, t, cond):
        c_txt = cond["c_txt"]
        c_img = cond["c_img"]
        #print(x_noisy.device, c_img.device, c_txt.device,t) #cpu cuda:0 cuda:0 tensor([999])
        x_noisy = x_noisy.to(c_img.device)
        t = t.to(c_img.device)
