    ...
    def convert_to_denoiser(
        ...
        def denoiser(x: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
            ...
            if uncond is None or cfg_scale == 1.0:
                ...
                #print(x.device, c_noise.device,c_in.device,c_out.device ) #cpu cpu cpu,cpu
               
                model_cond = model(x * c_in, c_noise, cond).to(x.device) * c_out + x * c_skip
                model_uncond = model(x * c_in, c_noise, uncond).to(x.device) * c_out + x * c_skip
