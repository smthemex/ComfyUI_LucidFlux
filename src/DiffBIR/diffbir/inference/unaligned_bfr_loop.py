...
class UnAlignedBFRInferenceLoop(InferenceLoop):
    def to(self, device,dtype=None):
        if dtype is not None:
            ...
            self.bg_cleaner.to(device,dtype) 
            self.face_cleaner.to(device,dtype)
            if self.cond_fn is not None:
                self.cond_fn.to(device,dtype)
        else:
            self.diffusion.to(device)
            self.cldm.to(device)
            self.bg_cleaner.to(device)
            self.face_cleaner.to(device,dtype)
            if self.cond_fn is not None:
    ...
    def load_cleaner(self) -> None:
        ...
        if self.args.swinir_face is  None:
            ...
            model_weight = load_model_from_file(self.args.swinir_face)
    ...
    def load_lq_(self,lq) -> Generator[Image.Image, None, None]:
