...
class InferenceLoop:
    ...
    def load_cldm(self) -> None:
        self.cldm: ControlLDM = instantiate_from_config(
            OmegaConf.load(self.args.cldm_config)
        )

        # load pre-trained SD weight
        if self.args.version == "v2.1":
            if self.args.sd_v2_zsnr is None:
                sd_weight = load_model_from_url(MODELS["sd_v2.1"])
            else:
                sd_weight = load_model_from_file(self.args.sd_v2_zsnr)
        else:
            # v1, v2
            sd_weight = load_model_from_url(MODELS["sd_v2.1"])
        ...
        if self.args.version == "v1":
            ...
            # v2.1
            if self.args.control_model is None:
                control_weight = load_model_from_url(MODELS["v2.1"])
            else:
                control_weight = load_model_from_file(self.args.control_model)
        ...
        if self.args.version in ["v1", "v2"]:
            ...
            config=self.args.diffusion_config
    ...
    def load_lq(self) -> Generator[Image.Image, None, None]:
        if isinstance(self.args.inputs, Image.Image):
            self.loop_ctx["file_stem"] = "output"
            yield self.args.inputs.convert("RGB")
        elif isinstance(self.args.inputs, list) and all(isinstance(img, Image.Image) for img in self.args.inputs):
            for i, img in enumerate(self.args.inputs):
                self.loop_ctx["file_stem"] = f"output_{i}"
            ...
            for file_name in sorted(os.listdir(self.args.inputs)):
                stem, ext = os.path.splitext(file_name)
                if ext not in img_exts:
                    print(f"{file_name} is not an image, continue")
                    continue
                file_path = os.path.join(self.args.inputs, file_name)
    ...
    @torch.no_grad()
    def run(self,input_image=None) -> None:
        self.setup()
        auto_cast_type = {
            "fp32": torch.float32,
            "fp16": torch.float16,
        ...
        if input_image is not None:
            if isinstance(input_image, Image.Image):
                lq_images = [input_image.convert("RGB")]
                self.loop_ctx["file_stem"] = "output"
                if self.args.task == "unaligned_face" :
                    lq_images=list(self.load_lq_(lq_images[0]))  
            elif isinstance(input_image, list):
                lq_images = [img.convert("RGB") for img in input_image]
            else:
                raise ValueError("input_image must be PIL Image or list of PIL Images")
        else:
            # 原有的逻辑
            lq_images = list(self.load_lq())

        all_pil_results = []

        for i, lq in enumerate(lq_images):
            # prepare prompt
            ...
            pil_results =self.return_as_pil(samples)
            all_pil_results.extend(pil_results)
            #self.save(samples, pos_prompt, neg_prompt)
        return all_pil_results  
    ...
    def return_as_pil(self, samples: List[np.ndarray]) -> List[Image.Image]:
        """
        将样本转换为 PIL 图像列表并返回，不保存到磁盘
        """
        pil_images = []
