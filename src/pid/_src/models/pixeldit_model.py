...
from ..._ext.imaginaire.lazy_config import instantiate as lazy_instantiate
from ..._ext.imaginaire.model import ImaginaireModel
from ..._ext.imaginaire.utils import misc
from ..utils.context_parallel import broadcast as cp_broadcast
from ..utils.context_parallel import robust_broadcast
...
class PixelDiTModel(ImaginaireModel):
    ...
    def __init__(self, config: PixelDiTModelConfig):
        ...
        if config.precomputed_caption_embeddings_path:
            ...
            null_caption_embs_path = os.path.join(os.path.dirname(config.precomputed_caption_embeddings_path),"null_caption_embs.pt")
            self._null_caption_embs = None  if not os.path.exists(null_caption_embs_path) else torch.load(null_caption_embs_path).to("cuda",torch.bfloat16)
            logger.info(
