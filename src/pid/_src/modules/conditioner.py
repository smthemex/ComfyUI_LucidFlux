...
from ..._ext.imaginaire.utils.count_params import count_params, disabled_train
from ..utils.context_parallel import broadcast
    ...
    def __init__(self, **emb_models: Union[List, Any]):
        ...
        for n, (emb_name, emb_config) in enumerate(emb_models.items()):
            ...
            if isinstance(emb_config, dict):
                ...
                embedder.is_trainable = emb_config.get("is_trainable", True)
                embedder.dropout_rate = emb_config.get("dropout_rate", 0.0)
            else:
                embedder = emb_config
                # 确保必要的属性存在，如果缺失则设置默认值  # 硬编码模型
                if not hasattr(embedder, 'is_trainable'):
                    embedder.is_trainable = True
                if not hasattr(embedder, 'dropout_rate'):
                    embedder.dropout_rate = 0.0
            # assert isinstance(
